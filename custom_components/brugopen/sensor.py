...
async def async_setup_entry(
    ...
    if watched:
        ...
        for entity_entry in er.async_entries_for_config_entry(
            ...
            # unique_id format: brugopen_<bridge_id>_last_opened
            if not entity_entry.unique_id.endswith("_last_opened"):
                continue
            bridge_id = entity_entry.unique_id[
                len(DOMAIN) + 1 : -len("_last_opened")
            ]
    ...
    def _discover() -> None:
        new_entities = [
            BridgeLastOpenedSensor(coordinator, bridge_id)
